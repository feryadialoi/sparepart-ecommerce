  ...
  <v-app>
    ...
    <v-content v-if="!isLoading">
      <v-container>
        ...
        >
        <template>
          <v-data-table :headers="headers" :items="productCategories" sort-by="index" class="elevation-1">
            <template v-slot:top>
                ...
                <v-row>
                  ...
                  <v-col class="py-0" cols="12" lg="6" md="6" sm="12">
                    <v-layout justify-end>
                      <v-btn color="primary_red_1" dark class="ml-2 mb-1" @click="dialog = true">New Category</v-btn>
                    </v-layout>
                  </v-col>
                </v-row>
              </v-card>
...
export default {
  mounted() {
    Promise.all([this.loadProductCategory()]).then(() => {
  ...
  methods: {
    ...
    async updateProductCategory() {
      const requestUpdate = async (productCategory) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
        ...
        const url = `${apiBaseUrl}/api/product-categories/${productCategory._id}`
        const config = {
          headers: headers,
          method: 'PUT',
          url: url,
      ...
      if (validate) {
        ...
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestUpdate(productCategory)
        }
      } else {
        throw new Error('invalid input!')
      }
    },
    async saveNewProductCategory() {
      const requestSaveNew = async (productCategory) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
      ...
      if (validate) {
        ...
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestSaveNew(productCategory)
        }
      } else {
        throw new Error('invalid input!')
      }
    ...
    async searchDataTable() {
      ...
      const requestSearch = async () => {
        ...
        const url = `${apiBaseUrl}/api/product-categories?keyword=${this.search}`
        const config = {
          headers: headers,
          method: 'GET',
          url: url,
      ...
      if (checkToken === true) {
        ...
        this.productCategories = result.data.productCategories.map((productCategory, index) => ({
          index,
          ...productCategory,
        }))
      }
    ...
    confirmDelete() {
      this.deleteProductCategory().then(() => {
        const index = this.productCategories.indexOf(this.item)
        this.productCategories.splice(index, 1)
        this.productCategories = this.productCategories.map((productCategory, index) => ({
          ...productCategory,
          index,
        }))
    ...
    async deleteProductCategory() {
      const requestDelete = async (productCategory) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
      ...
      const productCategory = { ...this.item }
      const checkToken = await checkTokenExp()
      if (checkToken === true) {
        return requestDelete(productCategory)
      }
    },
    closeEditOrNew() {
      this.dialog = false
