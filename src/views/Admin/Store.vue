  ...
  <v-app>
    ...
    <v-content v-if="!isLoading">
      <v-container>
        ...
        >
        <template>
          <v-data-table :headers="headers" :items="stores" sort-by="index" class="elevation-1">
            <template v-slot:top>
                  ...
                  <v-col class="py-0" cols="12" lg="6" md="6" sm="12">
                    <v-layout justify-end>
                      <!-- <v-btn
                        ...
                        >import csv</v-btn
                      > -->
                      <v-btn color="primary_red_1" dark class="ml-2 mb-1" @click="dialog = true">New Store</v-btn>
...
export default {
  mounted() {
    Promise.all([this.loadStore(), this.loadDealer()]).then(() => {
  ...
  data: () => ({
    ...
    dealerRules: [(v) => !!v || 'Dealer is required'],
    ...
    headers: [
      ...
      // { text: 'Pic', value: 'pic' },
      // { text: 'User/Dealer', value: 'userId.name' },
  ...
  methods: {
    ...
    async searchDataTable() {
      ...
      const requestSearch = async () => {
        ...
        const url = `${apiBaseUrl}/api/stores?keyword=${this.search}`
        const config = {
          headers: headers,
          method: 'GET',
          url: url,
    ...
    async updateStore() {
      const requestUpdate = async (store, image) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
        ...
        data.append('pic', store.userId.name)
      ...
      if (validate) {
        const store = {
          ...
          pic: this.editedItem.userId.name,
          status: this.editedItem.status,
        }
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestUpdate(store, this.image)
        }
      } else {
    ...
    async saveNewStore() {
      const requestSaveNew = async (store, image) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
      ...
      if (this.image && validate) {
        ...
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestSaveNew(store, this.image)
        }
      } else {
    ...
    confirmDelete() {
      this.deleteStore().then(() => {
        const index = this.stores.indexOf(this.item)
    ...
    async deleteStore() {
      const requestDelete = async (store) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
