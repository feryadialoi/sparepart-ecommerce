  ...
  <v-app>
    ...
    <v-content v-if="!isLoading">
     ...
     <v-tooltip bottom>
                ...
                <v-tooltip v-if="item.name !== 'admin'" bottom>
                  <template v-slot:activator="{ on }">
    ...
    <v-dialog v-model="dialogImport" max-width="500px">
      <v-card>
          ...
          <v-container>
            <v-row>
              <v-col cols="12" lg="12" md="12" sm="12">
                <div>
                  Download template untuk import data dalam bentuk Microsoft
                  Excel (xls)
                </div>
              </v-col>
              <v-col cols="12" lg="12" md="12" sm="12">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12" lg="12" md="12" sm="12">
                <div>
                  Upload data menggunakan file berformat csv (jika menggunakan
                  file Microsoft Excel/xlsx di-save as ke file berformat csv)
                </div>
              </v-col>
              <v-col v-if="csv" cols="12" sm="12" md="12" class="text-center">
                <v-icon size="100" color="primary_red_1"
                  >mdi-file-document</v-icon
                >
                <div>{{ csvName }}</div>
              </v-col>
        ...
        <v-card-actions class="pa-5">
          <v-spacer></v-spacer>
          <v-btn outlined color="blue darken-1" dark @click="closeDialogImport"
            >Cancel</v-btn
          >
          <v-btn outlined @click="submitImport" color="success darken-1"
            >import</v-btn
          >
        </v-card-actions>
      </v-card>
...
export default {
  mounted() {
    Promise.all([this.loadRole()]).then(() => {
  ...
  methods: {
    ...
    async searchDataTable() {
      ...
      const requestSearch = async () => {
        ...
        const url = `${apiBaseUrl}/api/roles?keyword=${this.search}`;
        const config = {
          headers: headers,
          method: 'GET',
          url: url,
    ...
    async updateRole() {
      const requestUpdate = async role => {
        const { token } = JSON.parse(localStorage.getItem('auth'));
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
        ...
        const data = role;
        const url = `${apiBaseUrl}/api/roles/${role._id}`;
        const config = {
          headers: headers,
          method: 'PUT',
          url: url,
    ...
    async saveNewRole() {
      const requestSaveNew = async role => {
        const { token } = JSON.parse(localStorage.getItem('auth'));
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
    ...
    confirmDelete() {
      this.deleteRole().then(() => {
        const index = this.roles.indexOf(this.item);
