  ...
  <v-app>
    <nav>
      <Navbar main="tes main">
        ...
        <template v-slot:body>
          <v-toolbar-title>Depot</v-toolbar-title>
        </template>
      </Navbar>
    </nav>
    ...
    <v-content v-if="!isLoading">
      <v-container>
        ...
        >
        <v-data-table :headers="headers" :items="depots" sort-by="index" class="elevation-1">
          <template v-slot:top>
                ...
                <v-col class="py-0" cols="12" lg="6" md="6" sm="12">
                  <v-layout justify-end>
                    <!-- <v-btn
                      ...
                      >import csv</v-btn
                    > -->
                    <v-btn color="primary_red_1" dark class="ml-2 mb-1" @click="dialog = true">New Item</v-btn>
                  </v-layout>
                </v-col>
              </v-row>
            </v-card>
          ...
          <template v-slot:item.storeId.name="{ item }">
            <div>
              {{ item.storeId == null ? 'Belum ada toko' : item.storeId.name }}
            </div>
          </template>
            ...
            <div class="no-services">
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-icon v-on="on" small class="mr-2" @click="openDialogChangePassword(item)">mdi-lock</v-icon>
                </template>
          ...
          <v-container>
            <v-row>
              <div>
                ...
                <span class="font-weight-bold">{{ `"${this.changePassword.name}"` }}</span>
              </div>
            </v-row>
...
export default {
  mounted() {
    Promise.all([this.loadDepot(), this.loadRole()]).then(() => {
  ...
  methods: {
    ...
    async searchDataTable() {
      const requestSearch = async () => {
        ...
        const url = `${apiBaseUrl}/api/users?role=depot&keyword=${this.search}`
        const config = {
          headers: headers,
          method: 'GET',
          url: url,
      ...
      if (checkToken === true) {
        ...
        this.depots = result.data.users.map((depot, index) => ({
          ...depot,
          index,
        }))
      }
    ...
    async saveChangePassword() {
      ...
      try {
        ...
        if (validate) {
          const result = await requestSavePassword(this.changePassword)
    ...
    async updateDepot() {
      const requestUpdate = async (depot, image) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'multipart/form-data',
        ...
        data.append('name', depot.name)
        data.append('role', depot.role)
        data.append('email', depot.email)
        data.append('phone', depot.phone)
        const url = `${apiBaseUrl}/api/users/${depot._id}`
        const config = {
          headers: headers,
          method: 'PUT',
          url: url,
      ...
      if (validate) {
        ...
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestUpdate(depot, this.image)
        }
      } else {
    ...
    async saveNewDepot() {
      const requestSaveNew = async (depot, image) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'multipart/form-data',
        ...
        data.append('email', depot.email)
        data.append('phone', depot.phone)
        data.append('password', depot.password)
      ...
      if (this.image && validate) {
        ...
        const checkToken = await checkTokenExp()
        if (checkToken === true) {
          return requestSaveNew(depot, this.image)
        }
      } else {
    ...
    async loadDepot() {
      const requestDealer = async () => {
        ...
        const url = `${apiBaseUrl}/api/users?role=depot`
        const config = {
          headers: headers,
          method: 'GET',
          url: url,
    ...
    async loadRole() {
      ...
      if (checkToken === true) {
        ...
        //   index,
        // }));
        this.roles = result.data.roles.filter((role) => role.name === 'depot')
        // return requestDealer();
      }
    },
    ...
    deleteItem(item) {
      this.item = item
      const index = this.depots.indexOf(item)
      this.deleteDialog = true
    },
    confirmDelete() {
      this.deleteDealer().then(() => {
        const index = this.depots.indexOf(this.item)
        this.depots.splice(index, 1)
        this.depots = this.depots.map((depot, index) => ({
          ...depot,
          index,
        }))
    ...
    async deleteDealer() {
      const requestDelete = async (depot) => {
        const { token } = JSON.parse(localStorage.getItem('auth'))
        const headers = {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
      ...
      const depot = { ...this.item }
      const checkToken = await checkTokenExp()
      if (checkToken === true) {
        return requestDelete(depot)
      }
    },
    closeEditOrNew() {
      this.dialog = false
